...
const PropertyTenants = () => {
  ...
  // Wait for all three queries before rendering — partial data would leave
  // the payment status column unable to resolve correctly.
  if (propertyLoading || leasesLoading || paymentsLoading) return <Loading />;

  // Scans the payment records for a due date that falls in the current
  // calendar month AND year. Both checks are required: getMonth() alone
  // would match the same month across different years (e.g. May 2024 vs
  // May 2025), producing a false "Paid" status on anniversary months.
  // Defaults to "Not Paid" when no matching payment record exists yet.
